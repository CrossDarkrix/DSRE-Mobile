...
def fail(s:str,code:int=2) -> None:
...
def detect_host_tag()  ->  str:
    ...
    if s == 'linux':
        return 'linux-x86_64'
    if s == 'darwin':
        return 'darwin-arm64' if m in ('arm64','aarch64') else 'darwin-x86_64'
    if s == 'windows' or s.startswith('msys') or s.startswith('cygwin'):
...
def resolve_ndk(v:Optional[str],c:Dict[str,object]) -> Path:
    ...
    if not raw:
        fail('Android NDK path is not set. Use --ndk, ANDROID_NDK, NDK, or configure first.')
...
def env_for(c:Dict[str,object], a:argparse.Namespace, abi:str)  ->  Dict[str,str]:
    e=os.environ.copy()
    ndk=resolve_ndk(getattr(a,'ndk',None),c)
    e['ANDROID_NDK']=str(ndk)
    e['NDK']=str(ndk)
...
def cmd_configure(a):
    c = load_config()
    if a.ndk or os.environ.get('ANDROID_NDK') or os.environ.get('NDK'):
        c['ndk'] = str(resolve_ndk(a.ndk,c))
    c['host_tag'] = a.host_tag or str(c.get('host_tag') or detect_host_tag())
    if a.android_api:
        c['android_api'] = str(a.android_api)
    c['android_platform'] = a.android_platform
    c['build_type'] = a.build_type
    if a.ffmpeg_source:
        c['ffmpeg_source'] = str(Path(os.path.expanduser(a.ffmpeg_source)).resolve())
    if a.lame_root:
        c['lame_root'] = str(Path(os.path.expanduser(a.lame_root)).resolve())
    save_config(c)

def cmd_ffmpeg(a):
    c = load_config()
    src_raw = a.ffmpeg_source or c.get('ffmpeg_source')
    if not src_raw:
...
def cmd_sync(a):
    c = load_config()
    for abi in parse_abis(a.abis or a.abi):
        sync_ffmpeg(ffmpeg_prefix(a, c, abi), abi, a.dry_run)
...
def cmd_clean(a):
    for p in [ROOT / 'android-build', ROOT / 'native_libs', ROOT / 'dsre_native' / 'ffmpeg', ROOT / 'dsre_native' / 'build-arm64-v8a', ROOT / 'dsre_native' / 'build-armeabi-v7a']:
        if p.exists():
            say(f'remove {p}')
            (None if a.dry_run else shutil.rmtree(p))

def add_common(p):
    p.add_argument('--ndk')
    p.add_argument('--host-tag')
    p.add_argument('--android-api')
    p.add_argument('--android-platform',default='android-24')
    p.add_argument('--build-type',default='Release')
    p.add_argument('--dry-run',action='store_true')

def make_parser():
    p = argparse.ArgumentParser(description='Unified DSRE-Mobile Android build driver')
    sub=p.add_subparsers(dest='command', required=True)
    c = sub.add_parser('configure', help='write machine-local build.local.json')
    c.add_argument('--ndk')
    c.add_argument('--host-tag')
    c.add_argument('--android-api')
    c.add_argument('--android-platform', default='android-24')
    c.add_argument('--build-type', default='Release')
    c.add_argument('--ffmpeg-source')
    c.add_argument('--lame-root')
    c.set_defaults(func=cmd_configure)
    f = sub.add_parser('ffmpeg', help='build FFmpeg for one or more ABIs')
    add_common(f)
    f.add_argument('--abi', choices=SUPPORTED_ABIS)
    f.add_argument('--abis')
    f.add_argument('--ffmpeg-source')
    f.add_argument('--lame-prefix')
    f.add_argument('--lame-root')
    f.add_argument('--ffmpeg-prefix')
    f.add_argument('--sync-native', action='store_true')
    f.set_defaults(func=cmd_ffmpeg)
    s = sub.add_parser('sync-ffmpeg', help='copy FFmpeg output into dsre_native/ffmpeg/<ABI>')
    add_common(s)
    s.add_argument('--abi',choices=SUPPORTED_ABIS)
    s.add_argument('--abis')
    s.add_argument('--ffmpeg-prefix')
    s.set_defaults(func=cmd_sync)
    n = sub.add_parser('native', help='build libdsre_audio.so')
    add_common(n)
    n.add_argument('--abi',choices=SUPPORTED_ABIS)
    n.add_argument('--abis')
    n.set_defaults(func=cmd_native)
    a = sub.add_parser('all', help='build FFmpeg, sync it, then build native libraries')
    add_common(a)
    a.add_argument('--abis',default='all')
    a.add_argument('--ffmpeg-source')
    a.add_argument('--lame-prefix')
    a.add_argument('--lame-root')
    a.add_argument('--ffmpeg-prefix')
    a.set_defaults(func=cmd_all)
    cl = sub.add_parser('clean', help='remove generated build outputs')
...
def main(argv:Optional[List[str]]=None) -> int:
    ...
    try:
        ...
        fail(f"command failed with exit code {e.returncode}: {' '.join(e.cmd)}",e.returncode or 1)
